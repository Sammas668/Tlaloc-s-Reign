...
#
# v0.43.1 extraction target.
# Owns Prestige calculation and summary rules while TRGameState remains the live
# state owner during the CampaignState migration.
...
func record_savvy_trade_prestige(state: Node, trade_lines: Array, detail: String = "Savvy market trade") -> Dictionary:
	...
	var player_prestige: float = float(state.get("player_prestige")) if state != null else 0.0
	if amount <= 0.001:
	...
	if report_variant is Array:
		...
		report.append(detail + ": +" + _format_amount(amount) + " Prestige from savvy trade.")
		state.set("last_report", report)
	if state != null and state.has_signal("state_changed"):
		state.emit_signal("state_changed")
	return {"ok": true, "amount": amount, "prestige": float(state.get("player_prestige")), "preview": preview, "record": result.get("record", {})}
...
func get_economic_prestige_summary(state: Node) -> Dictionary:
	...
	if state != null and state.has_method("get_prestige_history"):
		history = state.call("get_prestige_history") as Array[Dictionary]
	history.reverse()
...
func get_prestige_leaderboard(state: Node) -> Array[Dictionary]:
	...
	if state != null:
		var rival_variant: Variant = state.get("rival_prestige")
		if rival_variant is Dictionary:
			rival_prestige = (rival_variant as Dictionary).duplicate(true)
	if rival_prestige.is_empty():
		rival_prestige = default_rival_prestige_values()
		if state != null:
			state.set("rival_prestige", rival_prestige.duplicate(true))
...
func get_prestige_summary(state: Node) -> Dictionary:
	...
	if state != null and state.has_method("get_prestige_history"):
		history_rows = state.call("get_prestige_history") as Array[Dictionary]
	...
	return {
		...
		"prestige_history": state.call("get_prestige_history") if state != null and state.has_method("get_prestige_history") else [],
...
func get_rival_prestige(state: Node) -> Dictionary:
	var rival_values: Dictionary = {}
	if state != null:
		var rival_variant: Variant = state.get("rival_prestige")
		if rival_variant is Dictionary:
			rival_values = rival_variant as Dictionary
	if rival_values.is_empty():
		rival_values = default_rival_prestige_values()
		if state != null:
			state.set("rival_prestige", rival_values.duplicate(true))
