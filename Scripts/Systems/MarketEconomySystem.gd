...
#
# v0.45.5 extraction target.
# Owns market/village economy presentation and scarcity projection logic while
# TRGameState remains the public UI API during the CampaignState migration.
...
func base_market_goods(state: Node) -> Array[Dictionary]:
	...
	if state == null:
		return output
	var order: Array[String] = _string_array_from_variant(state.get("resource_order"))
	var resources: Dictionary = _dictionary_from_variant(state.get("resources"))
	var market_demand: Dictionary = _dictionary_from_variant(state.get("market_demand"))
...
func _market_economy(state: Node) -> Dictionary:
	if state == null:
		return {}
	var value: Variant = state.get("market_economy")
	if value is Dictionary:
		return value as Dictionary
	return {}

func _market_stock(state: Node, resource_id: String) -> float:
	if state != null and state.has_method("_ensure_campaign_state_stockpile_bridge"):
		var runtime_variant: Variant = state.call("_ensure_campaign_state_stockpile_bridge")
		if runtime_variant is Object:
			var runtime_object: Object = runtime_variant as Object
			if runtime_object.has_method("get_market_stock"):
				return float(runtime_object.call("get_market_stock", resource_id))
	if state != null:
		var stock_variant: Variant = state.get("market_stockpiles")
